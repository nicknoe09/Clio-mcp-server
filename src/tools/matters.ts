...
export function registerMatterTools(server: McpServer): void {
  ...
  server.tool(
    "create_matter",
    "Open (create) a new matter in Clio via POST /matters. Requires client_id (the matter's client — an existing contact). All other fields are optional: description, status, open_date, billing_method, practice_area_id, responsible_attorney_id, originating_attorney_id. Custom intake fields (e.g. Cause #, Court, Bill Frequency, Paralegal) can be set in the same call via custom_fields. Billing rates can be set via matter_rate (a single matter-wide hourly/flat rate) and/or user_rates (per-timekeeper rates, e.g. $300 for one attorney, $250 for another) — these are written through Clio's custom_rate association with a POST-then-PATCH, so if the rate PATCH fails the matter is still created and the rate error is reported. Use list_custom_fields(parent_type='Matter') for fields, get_contacts for client_id, and get_users for attorney/timekeeper user IDs. Reads the matter back after creation; surfaces Clio validation errors verbatim.",
    {
      ...
      rate_type: z
        ...
        .optional()
        .default("hourly")
        .describe("Type of rate for matter_rate/user_rates. 'hourly' (default) → Clio HourlyRate; 'flat' → FlatRate (a flat fee; Clio flips billing_method to 'flat' as a side effect)."),
      matter_rate: z
        .coerce
        .number()
        .optional()
        .describe("A single matter-wide rate (applied with no specific user). E.g. 300 for a $300/hr matter. Combine with user_rates for overrides, or use alone."),
      user_rates: z
        .array(
          z.object({
        ...
        .optional()
        .describe("Per-timekeeper rates on the matter, e.g. [{user_id: 123, rate: 300}, {user_id: 456, rate: 250}]. Each maps a user to their own rate (Clio's custom_rate.rates array)."),
      custom_fields: z
        .array(
          z.object({
            field_name: z
    ...
    async (params) => {
      try {
        ...
        // Apply billing rates via the custom_rate association (PATCH). The
        // matter already exists at this point, so a rate failure does NOT undo
        // creation — we report it alongside the created matter instead.
        const ratePayload = buildCustomRatePayload({
          rate_type: params.rate_type,
          matter_rate: params.matter_rate,
        ...
        if (ratePayload && created?.id) {
          try {
            ...
            rateResult = { applied: true, sent: ratePayload, custom_rate: rb?.custom_rate ?? null, billing_method: rb?.billing_method ?? null };
          } catch (rateErr: any) {
            rateResult = {
              applied: false,
              sent: ratePayload,
  ...
  server.tool(
    "set_matter_rate",
    "Set billing rate(s) on an existing matter via Clio's custom_rate association (PATCH /matters/{id}). Provide matter_rate (a single matter-wide rate) and/or user_rates (per-timekeeper rates, e.g. $300 for one attorney and $250 for another). rate_type selects 'hourly' (HourlyRate, default) or 'flat' (FlatRate — a flat fee, which flips the matter's billing_method to 'flat'). NOTE: the top-level billing_method field cannot be set directly — Clio always saves 'hourly' unless a FlatRate custom_rate is applied. Reads the matter back and returns the resulting custom_rate and billing_method. Use get_users to find user IDs.",
    {
      ...
      matter_rate: z
        ...
        .number()
        .optional()
        .describe("A single matter-wide rate (applied with no specific user), e.g. 300."),
      user_rates: z
        .array(
          z.object({
            user_id: z.coerce.number().describe("Clio user ID of the timekeeper. Find via get_users."),
        ...
        .optional()
        .describe("Per-timekeeper rates, e.g. [{user_id: 123, rate: 300}, {user_id: 456, rate: 250}]."),
    },
    async (params) => {
      const ratePayload = buildCustomRatePayload({
        rate_type: params.rate_type,
      ...
      if (!ratePayload) {
        return {
          content: [{
            ...
            text: JSON.stringify({
              error: true,
              message: "Provide matter_rate and/or at least one user_rates entry.",
            }),
          }],
          isError: true,
        };
      ...
      try {
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              success: true,
              matter: { id: rb?.id ?? params.matter_id, display_number: rb?.display_number ?? null },
              sent: ratePayload,
              billing_method: rb?.billing_method ?? null,
              custom_rate: rb?.custom_rate ?? null,
            }, null, 2),
          }],
        };
      } catch (err: any) {
