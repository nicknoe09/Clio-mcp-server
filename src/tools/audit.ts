      ...
      try {
        ...
        for (const e of flaggedEntries) {
          ...
          byBill.get(key)!.push({
            ...
            matter_description: e.matter_description,
            date: e.date,
        ...
        const result = {
          ...
          interactive_review_instructions: [
            "IMPORTANT: After showing the summary, ask the user if they want to interactively review and fix flagged entries one by one.",
            "If YES, present flagged entries ONE AT A TIME using this format for each entry:",
            "",
            "---",
            "**Entry [N] of [total]** | [severity emoji: 🔴 strike, 🟠 reduce, 🟡 review, 🔵 rephrase]",
            "",
            "📋 **Matter:** [matter_number] — [matter_description]",
            "👤 **Timekeeper:** [name] | 📅 **Date:** [date]",
            "⏱️ **Hours:** [hours] | 💰 **Rate:** $[rate]/hr | **Amount:** $[amount]",
            "",
            "📝 **Current Description:**",
            "> [full current note text]",
            "",
            "⚠️ **Issues Found:**",
            ...
            "> [suggested_description — if provided]",
            "",
            "**What would you like to do?**",
            "1️⃣ **Accept** suggested revision",
            "2️⃣ **Skip** — leave unchanged",
            "3️⃣ **Custom** — type your own revision",
            "---",
            "",
            "RULES FOR INTERACTIVE REVIEW:",
            "- Wait for the user's response before moving to the next entry.",
            "- If user picks 1 (Accept): call apply_entry_revision with the activity_id and suggested changes (new_note, new_rate, new_hours as applicable). Confirm success, then show the next entry.",
            "- If user picks 2 (Skip): acknowledge and immediately show the next entry.",
            "- If user picks 3 (Custom) or types a custom description: call apply_entry_revision with whatever the user typed, confirm success, then show the next entry.",
            "- If user types a number or short response, interpret it as the corresponding option.",
            "- After each entry is processed, show a running tally: '[N] reviewed, [accepted] accepted, [skipped] skipped, [custom] custom edits'.",
            "- After ALL entries are done, show a final summary of all changes made.",
            "- NEVER present more than one entry at a time. This prevents crashes from losing progress.",
            "- Each accepted change is saved immediately via apply_entry_revision — no batching.",
          ],
        };
