...
| **`get_calendar_entries` misses events someone else put me on** | By default `user_id` returns only entries on calendars that user **owns**. Events another person calendared and added them to as an attendee live on the *other* person's calendar, so they're excluded. Pass `include_attending=true` to also catch those — each entry then carries `match`: `"owner"`, `"attendee"`, or `"both"`. Clio has **no attendee query filter** (verified against the OpenAPI spec: the only ownership filter on `/calendar_entries` is `calendar_id`), so `include_attending` sweeps the date range and matches `attendees[]` client-side — keep the range tight. Note `CalendarEntry.attendees` is **plural** (an array of `Attendee_base`); a singular `attendee` field does not exist and asking for it makes Clio reject the request. `Attendee_base.type` is `"Contact"` or `"Calendar"` — a *person* attendee is identified by their **Calendar** id, so Contact attendees must never be compared against calendar ids. |
