...
| **Splitting a block-billed entry into multiple sub-entries** | Use `prepare_line_split(line_item_id, splits=[{hours, note}, …])`. It edits the existing line to the first sub-entry and creates new activities on the matter for the rest (unbilled, inheriting date/user/rate from the original). Then click "Regenerate Draft" on the bill in Clio UI to pull the new activities onto the bill — Clio's API has no endpoint that does that last step. Strict total: split hours must equal the original line's hours. |
