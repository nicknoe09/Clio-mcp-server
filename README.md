...
| Problem | Solution |
|---------|----------|
| **Calendar entries: custom event types / cross-calendar assignment don't apply** | The custom calendaring behavior — RomSum/NRN event types and assigning to another user's calendar (`event_type` / `event_type_id` / `calendar_owner_id` / `assign_to_user_id`) — is **owner-only**. Set `OWNER_EMAILS` (comma-separated, defaults to the original author) to control who gets it. For every other user, `create_calendar_entry` / `update_calendar_entry` ignore those inputs and the event simply lands on the acting user's own personal Clio calendar; the response lists what was ignored. |
| **Calendar entry lands on the wrong calendar (owner only)** | `calendar_owner` on a CalendarEntry is a **Calendar ID**, not a User ID — these are separate Clio resources. Passing a User ID where Clio expects a Calendar ID returns 404. To put an event on a specific user's personal calendar, either: (a) use `assign_to_user_id` (looks up the user's Calendar resource automatically); or (b) use `list_calendars(creator_user_id=N)` to find the right Calendar ID and pass it via `calendar_owner_id`. With no targeting param, the event defaults to the acting user's own personal calendar. |
